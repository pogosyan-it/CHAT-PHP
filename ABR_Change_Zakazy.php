...
$index=0; 
$DateOfRequest = date("Y-m-d").'_Zakazy.txt';
$head="Номер Заказа       Маршрут          Дата                  Изменение региона            Способ отправки                  Примечание";
$fd = fopen("/var/www/files/Zakazy/$DateOfRequest",'a+') or die("не удалось создать файл");
$size = filesize("/var/www/files/Zakazy/$DateOfRequest"); 
 ...
 #fwrite($fd, iconv("cp1251", "utf-8", $size."\r\n"));
     ...
     $result = mysqli_query($link,  "Select d15_departures.WayBillNum,  d15_departures.SY_Adding,   d15_departures.ID, 
                                     a.Name as 'dest_adr', b.Name as 'dest_region', hbc_rChange.Region_abr, d15_departures.PickUpCode, 
												             hbc_routes.Name, d00_buff.str67, d00_buff.str0F from d15_departures 
                                    ...
                                    where hbc_cities.id  in (Select hbc_rChange.id_city_adr from hbc_rChange) and a.Name<>b.Name 
                                    and d15_departures.WarehousID=0 and d15_departures.WayBillNum IS NULL and d15_departures.SY_Void=0 and b.Name 
                  ...
                  if  ($row[4] == $row[5] )    {
                   ...
                   fwrite($fd, iconv("cp1251", "utf-8", '  '.$row[6].'              '.$row[7].'         '.$row[1].'            ' .$row[4].'   =>    '.$row[3].'            '.$row[9].'               '.$row[8]."\r\n"));  }
